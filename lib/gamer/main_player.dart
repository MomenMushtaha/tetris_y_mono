...
const GAME_PAD_HEIGHT = 20;

///the width of game pad
const GAME_PAD_WIDTH = 10;
...
class GameControl extends State<Game> with RouteAware {
  GameControl() {
    ...
    for (int i = 0; i < GAME_PAD_HEIGHT; i++) {
      _data.add(List.filled(GAME_PAD_WIDTH, 0));
      _mask.add(List.filled(GAME_PAD_WIDTH, 0));
    }
  }
    ...
    if (clearLines.isNotEmpty) {
      ...
      for (int count = 0; count < 5; count++) {
        clearLines.forEach((line) {
          _mask[line].fillRange(0, GAME_PAD_WIDTH, count % 2 == 0 ? -1 : 1);
        });
        setState(() {});
        await Future.delayed(Duration(milliseconds: 100));
      }
      clearLines.forEach((line) => _mask[line].fillRange(0, GAME_PAD_WIDTH, 0));

      clearLines.forEach((line) {
        _data.setRange(1, line + 1, _data);
        _data[0] = List.filled(GAME_PAD_WIDTH, 0);
      });
  ...
  static void _forTable(dynamic function(int row, int column)) {
    for (int i = 0; i < GAME_PAD_HEIGHT; i++) {
      for (int j = 0; j < GAME_PAD_WIDTH; j++) {
  ...
  void reset() {
    ...
    () async {
      int line = GAME_PAD_HEIGHT;
      await Future.doWhile(() async {
        line--;
        for (int i = 0; i < GAME_PAD_WIDTH; i++) {
          _data[line][i] = 1;
        }
        setState(() {});
  ...
  Widget build(BuildContext context) {
    List<List<int>> mixed = [];
    for (var i = 0; i < GAME_PAD_HEIGHT; i++) {
      mixed.add(List.filled(GAME_PAD_WIDTH, 0));
      for (var j = 0; j < GAME_PAD_WIDTH; j++) {
